...
let reverse lst =
  let rec reverse_aux acc lst = match acc, lst with
    | acc, [] -> acc
    | acc, x :: xs -> reverse_aux (x :: acc) xs
  in reverse_aux [] lst
...
let repeat str n = 
  let rec repeat_aux acc str n = match acc, str, n with
    | acc, str, 0 -> acc
    | acc, str, _ -> repeat_aux (str :: acc) str (n - 1)
  in repeat_aux [] str n
...
let rec range n =
  let rec range_aux acc n = match n with
    | 0 -> acc
...
let map f list =
  let rec map_aux acc f list = match list with
    | [] -> reverse acc 
...
let mapi f list =
  let i = 0 in
  let rec mapi_aux acc f list i= match list with
    | [] -> reverse acc
    | x :: xs -> mapi_aux ((f x i) :: acc) f xs (i + 1)
  in mapi_aux [] f list i
...
let zip list1 list2 = 
  let rec zip_aux acc list1 list2 = match list1, list2 with
    | [], [] -> reverse acc
    | [], _ -> failwith "Different lengths of input lists."
    | _, [] -> failwith "Different lengths of input lists."
    | x :: xs, y :: ys -> zip_aux ((x, y) :: acc) xs ys
  in zip_aux [] list1 list2
